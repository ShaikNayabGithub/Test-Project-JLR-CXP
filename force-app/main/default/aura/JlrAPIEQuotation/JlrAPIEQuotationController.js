({
    doInit: function (component, event, helper) {
        ...
        let action= component.get("c.GetCountryLanguage");
        action.setParams({"opptyId":recordId});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === 'SUCCESS') {
               
                const result=response.getReturnValue();
                component.set("v.language1", result);
            }else if(state === "ERROR"){
    ...
    getTradeInVinJs : function (component, event, helper) {
        ...
        if (searchString.length >= 3) {
            ...
            var inputTimer = setTimeout($A.getCallback(function () {
                helper.searchTradeInVin(component, searchString, objQuote.Brand__c);
            }), 1000);
    ...
    handleSubmitJs : function (component, event, helper) {
        ...
        const mdName=component.get("v.quoteInitWrap.ModelName");
        ...
        var lang=component.find('language').get('v.value');
        
        let compLst=QuoteWrap.complementaryProdLst;
        let action= component.get("c.saveQuoteApex");
        action.setParams({"Quote": quoteObj,"complementaryProdStr":JSON.stringify(QuoteWrap.complementaryProdLst),"MdName":mdName});
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === 'SUCCESS') {
                
                const result=response.getReturnValue();
                helper.generateDoc(component, event, helper,result,lang);     
